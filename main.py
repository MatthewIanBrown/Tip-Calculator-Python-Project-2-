#Tip Calcuator

#If the bill was $150.00, split between 5 people, with 12% tip. 

#Each person should pay (150.00 / 5) * 1.12 = 33.6
#Format the result to 2 decimal places = 33.60

#Tip: There are 2 ways to round a number. You might have to do some Googling to solve this.💪

#Write your code below this line 👇

print("Welcome to the tip calculator!")

total_cost= float(input("What was the total bill? $"))



tip_amt= int(input("How much tip would you like to give? 10, 12, or 15?"))

tip_percent=float(1+(tip_amt/100))

num_of_ppl= int(input("How many people to split the bill?"))

per_person_amt= (total_cost*tip_percent)/num_of_ppl

final_amt = "{:.2f}".format(per_person_amt)

result= print(f"Each person should pay: ${final_amt}")